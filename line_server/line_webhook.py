...
def _extract_paid_amount(comments: str, total_amount: float) -> dict:
    ...
    try:
        client = _gemini_client()
        prompt = (
            "你是一個對帳助手。以下是 Trello 卡片上完整的留言紀錄（可能包含多筆對話、銀行轉帳通知等，"
            "依時間排序，越舊的留言可能在越前面）。\n"
            "請判斷裡面是否有明確提到「客戶已經實際付款/匯款/收到」的具體金額。\n"
            "規則：\n"
            "1. 只採信「已經發生、已確認收到」的金額，不要採信報價、應收、預計、詢問中的金額。\n"
            "2. 若同一筆款項留言中**同時出現百分比和實際匯款數字**（例如「收到50%訂金」"
            "後面接著「匯款NT207,360」），優先採用留言中明確寫出的實際數字，"
            "不要自己用總金額×百分比重新計算，因為實際入帳金額才是最準確的依據"
            "（可能因匯率、手續費、議價等因素跟理論百分比金額有些微差異）。\n"
            "3. 只有當留言「只」提到百分比、完全沒有列出實際數字時，才用總金額換算成實際數字。\n"
            "4. 若有多筆已確認收到的款項（例如訂金、尾款分次入帳），加總全部已收金額。\n"
            "5. 找不到明確已收金額時，found 填 false，paid_amount 填 0。\n"
            "6. 只回傳 JSON 物件，格式："
